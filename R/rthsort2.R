...
#' 
#' @export
rthsort2 <- function(x, decreasing=FALSE, inplace=FALSE, nthreads=rth.nthreads())
{
  nthreads <- as.integer(nthreads)
  
  if (is.integer(x)) {
     out <- .Call("rthsort2_int", x, as.integer(decreasing), 
  ...
  else
  {
    ...
    out <- .Call("rthsort2_double", x, as.integer(decreasing), 
              as.integer(inplace), as.integer(nthreads), PACKAGE="Rth")
  }
